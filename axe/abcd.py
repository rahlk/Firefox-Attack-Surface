...
class Abcd:
  ...
  def __init__(i, db="all", rx="all", show=False):
    i.db = db
    i.rx = rx
    i.show = show
    i.yes = i.no = 0
  ...
  def header(i):
    if i.show:
      print "#", ('{0:20s} {1:11s}  {2:4s}  {3:4s} {4:4s} ' +
                  ...
                  '{10:3s} {11:3s}{12:3s}            {13:30s}').format(
          "db", "rx",
  ...
  def ask(i, raw=True):
    ...
    for x in i.known:
      ...
      return [d, c, b, a, p(acc), p(pd), p(pf), p(prec), p(f), p(g)]


def _Abcd(before=None, after=None, show=False):
  abcd = Abcd(db='Actual', rx='Predicted', show=show)
  train = before
  test = after
  # random.shuffle(test)
